...
- Consent UI is recognized by vendor or naming (OneTrust, Sourcepoint, Didomi, iubenda, Quantcast, Usercentrics, Cookiebot, TrustArc, `gdpr`, privacy walls/banners/centers) or, for in-house banners, as a fixed/sticky dialog whose text asks for cookie or personal-data consent; the latter is sent as a *possible* consent overlay for the model to confirm. Only containers qualify, so a heading, link or script named "cookie" (including a site's consent-settings link) stays visible. Modals appended at the end of large pages are checked first.
- In automatic mode, a consent banner that mounts after its template was saved triggers one re-analysis of that template. The attempt is recorded per analysis version, so a banner the model keeps never triggers it again; **Forget this page type** clears it.
